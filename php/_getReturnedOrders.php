...
try{
  ...
  $query = "select orders.*,DATEDIFF('".date('Y-m-d')."', date_format(orders.date,'%Y-%m-%d')) as days,
            ...
            ";
  $where = "where";
  $filter = "orders.client_id =".$_SESSION['userid']." and (orders.order_status_id=9 or orders.order_status_id=6)  and (orders.confirm=1 or orders.confirm=4) and storage_id <> 1 and storage_id <> -1";
  if(!empty($search)){
